...
class RegisterViewModelTest {
    ...
    fun `blank email sets Error without calling the repository`() {
        viewModel.signUp(email = "", password = "secret123", confirmPassword = "secret123")
    ...
    fun `blank password sets Error without calling the repository`() {
        viewModel.signUp(email = "user@example.com", password = "", confirmPassword = "")
    ...
    fun `password shorter than 6 characters sets Error without calling the repository`() {
        viewModel.signUp(email = "user@example.com", password = "abc12", confirmPassword = "abc12")
    ...
    fun `mismatched passwords set Error without calling the repository`() {
        viewModel.signUp(email = "user@example.com", password = "secret123", confirmPassword = "different123")
    ...
    fun `successful sign up transitions from Loading to Success`() {
        ...
        viewModel.signUp(email = "user@example.com", password = "secret123", confirmPassword = "secret123")
        ...
        coVerify(exactly = 1) { authRepository.signUp("user@example.com", "secret123") }
    }

    @Test
    fun `failed sign up transitions from Loading to Error`() {
        coEvery { authRepository.signUp(any(), any()) } returns Result.failure(Exception("El correo ya está en uso"))
