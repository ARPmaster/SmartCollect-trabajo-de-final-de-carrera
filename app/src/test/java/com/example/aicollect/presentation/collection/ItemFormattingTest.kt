...
class ItemFormattingTest {
    ...
    @Test
    fun `formatValue returns Sin valorar for a null value`() {
        assertEquals("Sin valorar", ItemFormatting.formatValue(null, "EUR"))
    }

    @Test
    fun `formatValue formats EUR with the euro symbol after the amount`() {
        assertEquals("1.234€", ItemFormatting.formatValue(1234.0, "EUR"))
    }

    @Test
    fun `formatValue formats USD with the dollar symbol after the amount`() {
        assertEquals("100$", ItemFormatting.formatValue(100.0, "USD"))
    }

    @Test
    fun `formatValue falls back to the raw currency code for an unknown currency`() {
        assertEquals("50GBP", ItemFormatting.formatValue(50.0, "GBP"))
    }
